...
public class ConAir {
    ...
    public int frequentFliers() {
        ...
        for (int i = 0; i < numberOfPassengers; i++){
            ...
            for (int j = 0; j < numberOfFlights; j++){
                if (flights[j].containsPassenger(passengers[i])){
                    count++;
                }
            }
            if (count > 5){
    ...
    public boolean hasBadLayover(Passenger passenger, int layoverMins) {
        assert layoverMins >= 0;
        Flight[] flights = flights();

        if(passenger.flightCount() >= 2){
            for (int i = 0; i < flights.length; i++){
                for (int j = 0; j < flights.length; j++){
                    if (flights[i].containsPassenger(passenger) && flights[j].containsPassenger(passenger) &&
                    flights[i].destination().equals(flights[j].origin()) && flights[i].tightConnection(flights[j], layoverMins)){
                        assertInv();
                        return true;
                    }
                }
